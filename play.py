...
sys.path.insert(0,'./sunfish')
import sunfish
def make_to_move(nextmove,board):
	rows = "abcdefgh"
	cols = ["1","2","3","4","5","6","7","8"]
	move = rows[nextmove[0][0]]+cols[nextmove[0][1]]+rows[nextmove[1][0]]+cols[nextmove[1][1]]
	for legal in board.legal_moves:
		if legal == chess.Move.from_uci(move):
...
def get_square(coor):
	...
	sqx = 7-(x-455)//61
	sqy = 7-(y-122)//61
	if sqx < 0 or sqx >= 8 or sqy < 0 or sqy >= 8:
		return None
	return (sqx,sqy)
...
class Computer(Player):
	...
	def move(self, gn_current):
		...
		while move not in gn_current.board().legal_moves:
			crdn = sunfish.render(best_move[0]) + sunfish.render(best_move[1])
			move = create_move(gn_current.board(), crdn)
...
class Human(Player):
	def move(self, gn_current):
		...
		while True:
			...
			move_list = get_move()
			if move_list:
				move = make_to_move(move_list,bb)
				if move != None:
					break
		print(move)
		gn_new = chess.pgn.GameNode()
