...
def get_training_model(Ws_s, bs_s, dropout=False, lambd=10.0, kappa=1.0):
	...
	xd_s, xd_p = load.get_model(Ws_s, bs_s, dropout=dropout)
	xa_s, xa_p = load.get_model(Ws_s, bs_s, dropout=dropout)

	#loss = -T.log(sigmoid(xc_p + xp_p)).mean() # negative log likelihood
	#loss += -T.log(sigmoid(-xp_p - xr_p)).mean() # negative log likelihood

	cd_diff = xd_p - xa_p
	loss_a = -T.log(sigmoid(cd_diff)).mean()
	...
	return xc_s, xp_s, xd_s, xa_s, loss, reg, loss_a, loss_b, loss_c
...
def get_function(Ws_s, bs_s, dropout=False, update=False):
	xc_s, xp_s, xd_s, xa_s, loss_f, reg_f, loss_a_f, loss_b_f, loss_c_f = get_training_model(Ws_s, bs_s, dropout=dropout)
	obj_f = loss_f + reg_f
	...
	f = theano.function(
		inputs=[xc_s, xp_s, xd_s, xa_s, learning_rate],
		outputs=[loss_f, reg_f, loss_a_f, loss_b_f, loss_c_f],
		updates=updates,
		on_unused_input = 'ignore')
	print 'finished compiling function'
	print f
	return f

def training():
	global train,test
	Xc_train, Xc_test, Xp_train, Xp_test, Xd_train, Xd_test, Xa_train, Xa_test = get_data(['x', 'xp', 'xd', 'xa'])
	...
	n_in = 12 * 64
	Xtrain = [x for x in numpy.concatenate(Xc_train),numpy.concatenate(Xp_train),Xd_train.flatten(),Xa_train.flatten()]
	Xtest = [x for x in numpy.concatenate(Xc_test),numpy.concatenate(Xp_test),Xd_test.flatten(),Xa_test.flatten()]
	...
	while True:
		...
		print learning_rate
		minibatch_index = random.randint(0, int(len(Xp_train) / minibatch_size) - 1)
		print minibatch_index

		lo, hi = minibatch_index * minibatch_size, (minibatch_index + 1) * minibatch_size
		print lo,hi

		loss, reg, loss_a, loss_b, loss_c = train(Xc_train[lo:hi], Xp_train[lo:hi], Xd_train[lo:hi], Xa_train[lo:hi], learning_rate)
		print loss
